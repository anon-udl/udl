...
from bayes_cnn import KerasMNISTCNN, random_acquisition, max_entropy, mean_std, bald, train
...
def bald(preds):
    ...
    EH = torch.sum(sum_t, dim=-1) / 1
    return H + EH # (B)

def variation_ratio(preds):
    p = torch.sum(preds, dim=0) / 1 # (B, C)
    maxy = torch.max(p, dim=-1).values # (B)
    return 1.0 - maxy

def max_entropy(preds): # preds shape = (T, B, C)
    scaled_sum_phat_tc = torch.sum(preds, dim=0) / 1 # (B, C)
    ...
    if acq_func == random_acquisition: # add random idx to pool
        ...
        with torch.inference_mode():
            ...
            for data, _ in pool_loader:
                ...
                # preds = torch.sum(preds, dim=0) / T # (B, C)
                preds = preds.unsqueeze(0) # (1, B, C)
                # print(preds.shape)
                # raise
...
if __name__ == "__main__":
    ...
    if args.a == "all": funcs = [random_acquisition, mean_std, max_entropy, variation_ratio, bald]
    else: 
        funcs = {"random_acquisition": random_acquisition, "mean_std": mean_std, "max_entropy": max_entropy, "variation_ratio": variation_ratio, "bald": bald}
        funcs = [funcs[args.a]]

    # hyperparameters
    NUM_ACQUIRE, NUM_EPOCHS, NUM_CLASSES, BATCH_SIZE, SEED, NUM_SEEDS, T, VERBOSE = 10, 50, 10, 128, 32, 1, 1, False
    DETERMINISTIC = True
    ...
    for _ in range(3):
        ...
        filename = f"exp_k3_{funcs[0].__name__}_s{SEED}_d{DETERMINISTIC}_STD.pkl" if len(funcs) == 1 else f"exp_k3_all_s{SEED}_d{DETERMINISTIC}.pkl"
